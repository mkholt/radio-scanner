...
function stubRequest(expected)
{
	...
	request.callsArgWith(1, responseStream)
...
describe('fetcher', function ()
{
	describe("fetch", function ()
	{
		...
		it("should throw an error when date is empty", function ()
		{
			...
			{
				fetcher.fetch();
			}).should.throw("Date cannot be empty");
		});
		...
		it("should throw an error when date is not a string", function ()
		{
			(function ()
			{
				fetcher.fetch(160205, "03");
			}).should.throw("Date must be a string, 'number' given");
		});
		...
		it("should throw an error when hour is empty", function ()
		{
			(function ()
			{
				fetcher.fetch("160205");
			}).should.throw("Hour cannot be empty");
		});
		...
		it("should throw an error when hour is not a string", function ()
		{
			(function ()
			{
				fetcher.fetch("160205", 3);
			}).should.throw("Hour must be a string, 'number' given");
		});
		...
		it("should call with data from file, when date / hour already fetched", sinon.test(function (done)
		{
			...
			this.stub(fs, "open")
			    .callsArgWith(2, openError);
			this.stub(fs, "readFile")
			    .callsArgWith(1, undefined, expected);
		...
		it("should attempt to fetch the URL, and attempt to write to file, passing the error", sinon.test(function (done)
		{
			...
			this.stub(fs, "open")
			    .callsArgWith(2, undefined, fd);
			this.stub(fs, "writeFile")
			    .callsArgWith(2, writeError);
	...
	describe("getData", function ()
	{
		...
		it("should throw an error when URL is empty", function ()
		{
			...
			{
				fetcher.getData()
			}).should.throw("URL cannot be empty");
		});
