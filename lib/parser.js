...
function parseArtist(artist) {
    ...
    if (split.length) {
        ...
        featuring = split
            .map((part) => part.split(/ ?& ?| ?and ?/))          // Split each featuring artist by and
            .reduce((previous, current) => previous.concat(current));   // Flatten the resulting array of arrays, so we just get a list
    }
